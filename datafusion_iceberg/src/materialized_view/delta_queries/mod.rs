...
",
    ...
    #[tokio::test]
    async fn test_materialized_view_incremental_aggregate_join() {
        let temp_dir = TempDir::new().unwrap();
...
",
    ...
    end) as high_line_count, O_ORDERSTATUS from warehouse.tpch.orders WHERE O_ORDERDATE >= date '1996-01-01' group by O_ORDERSTATUS;",
            )
            .await
            .expect("Failed to create plan for select");
...
GROUP BY
    l_shipmode;",
        ...
        let plan = ctx
            .state()
            .create_logical_plan(
                "CREATE TEMPORARY VIEW warehouse.tpch.query12 AS SELECT
    l_shipmode,
    sum(case
        when o_orderpriority = '1-URGENT'
            OR o_orderpriority = '2-HIGH'
...
GROUP BY
    l_shipmode;
        ...
        let batches = ctx
            .sql("select * from warehouse.tpch.query12;")
            .await
            .expect("Failed to create plan for select")
            .collect()
            .await
